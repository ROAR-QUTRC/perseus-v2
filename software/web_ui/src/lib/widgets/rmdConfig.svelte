...
<script lang="ts">
	...
	import type {
		...
		RMDStatusRequest,
		RMDStatusResponse,
		SuccessServiceResponse as SuccessResponse
	} from './rmdConfig/serviceTypes';
	...
	const callBrakeEnable = (enable: boolean) => {
		if (brakeEnableService) {
			...
			brakeEnableService.callService(request, (response: { success: boolean }) => {
				console.log(
					`Brake ${enable ? 'enabled' : 'disabled'} for motor ID ${selectedId}: ${response.success}`
				);
			});
		}
	};
...
<div class="flex">
	...
	<Button class="ml-auto" onclick={callDebugEnable}
		>{debugEnabled ? 'Disable ' : 'Enable '} Debug</Button
	>
</div>

<div class="mt-2 flex h-full flex-wrap gap-2">
	<div class="mb-2 w-1/4 min-w-[200px]">
		<div class="relative aspect-square w-full rounded-2xl border">
			{#if selectedId !== 0}
				<div
					class="absolute left-[50%] top-[50%] aspect-square w-2/3 translate-x-[-50%] translate-y-[-50%] content-center text-center"
				>
				...
				<div class="z-5 absolute left-[10%] top-[10%] aspect-square w-[80%] rounded-[50%] border">
					<span class="absolute left-1/2 h-[5%] translate-x-[-1px] border border-l-0 border-r"
					></span>
					<span
				...
				<svg viewBox="0 0 100 100" class="absolute left-0 top-0 z-10 h-full w-full">
					<circle
						r="3%"
			...
			{:else}
				<div
					class="absolute left-[50%] top-[50%] aspect-square w-1/2 translate-x-[-50%] translate-y-[-50%] content-center text-center"
				>
	...
	<div class="w-1/2 min-w-[400px]">
		<div style:aspect-ratio="2 / 1" class="relative flex w-full flex-col rounded-2xl border p-2">
			{#if selectedId !== 0}
						...
						<AlertDialog.Content>
							<AlertDialog.Header>
								...
								<AlertDialog.Description>
									This will disengage <strong>all</strong> motors breaks. You must ensure they are not
									under load.
								</AlertDialog.Description>
							</AlertDialog.Header>
							<AlertDialog.Footer>
	...
	<div class="mb-2 min-w-[200px]">
		...
		<p>
			<strong>Phase current</strong>:<br />a -> {motorStatus?.phase_a_current.toFixed(2)}, <br />b
			-> {motorStatus?.phase_b_current.toFixed(2)},<br />c -> {motorStatus?.phase_c_current.toFixed(
				2
			)}
		</p>
