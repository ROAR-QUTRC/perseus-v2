...
export const updateHUD = (
	...
	ctx.beginPath();
	ctx.arc(c.x, c.y, mainRadius * 1.05, pi * -0.75 - piHalf, pi * -0.25 - piHalf);
	ctx.arc(c.x, c.y, mainRadius * 1.3, pi * -0.25 - piHalf, pi * -0.75 - piHalf, true);
	ctx.arc(c.x, c.y, mainRadius * 1.05, pi * -0.75 - piHalf, pi * -0.75 - piHalf);
	ctx.stroke();
	ctx.beginPath();
	ctx.arc(c.x, c.y, mainRadius * 1.05, pi * 0.25 - piHalf, pi * 0.75 - piHalf);
	ctx.arc(c.x, c.y, mainRadius * 1.3, pi * 0.75 - piHalf, pi * 0.25 - piHalf, true);
	ctx.arc(c.x, c.y, mainRadius * 1.05, pi * 0.25 - piHalf, pi * 0.75 - piHalf);
	ctx.stroke();
	...
	// Horizon line
	ctx.beginPath();
	ctx.moveTo(c.x + mainRadius * Math.cos(horizonTilt), c.y + mainRadius * Math.sin(horizonTilt));
	ctx.lineTo(
	...
	for (let i = 0; i < 50; i++) {
		...
		if (Math.abs(line.x1 - c.x) > mainRadius - 10 || Math.abs(line.y1 - c.y) > mainRadius - 10) {
			continue;
		}
		if (Math.abs(line.x2 - c.x) > mainRadius - 10 || Math.abs(line.y2 - c.y) > mainRadius - 10) {
			continue;
		}
		ctx.beginPath();
