<script lang="ts">
    ...
    const toggleExpand = (event: MouseEvent, path: string, key: string, state: boolean, canCollapse: boolean) => {
    ...
    <div class="flex flex-col">
        {#each Object.entries(data) as [key, value]}
            ...
            <button onclick={(e) => toggleExpand(e, path, key, collapsed, containsObject || isArray)}>
                <div 
                    class={`${!collapsed && (containsObject || isArray) ? 'flex-col' : 'flex-row'} ${containsObject || isArray ? '' : 'cursor-auto'} ${depth > 0 ? ' border-l  pl-[8px]' : ''} rounded-r-lg flex justify-start text-left ml-[3px] mr-[16px] border-0 hover:bg-border`} 
                    style:--tw-bg-opacity="0.3">
                    <p class="flex flex-row">
                        {#if containsObject || isArray}
                            <Fa class="mt-[4px] mr-[8px]" icon={collapsed ? faCaretRight : faCaretDown} />
                        {/if}
                        <b>{key}:&nbsp;</b>
                        {#if !collapsed}
                            {#if containsObject}
                    ...
                    {:else}
                        {#if containsObject}
                            <p style:color="mediumpurple" style="display: inline-block;">&nbsp;&lbrace;&nbsp;...&nbsp;&rbrace;</p>
                        {:else if isArray}
                            <p style:color="mediumpurple" style="display: inline-block;">&nbsp;&lbrack;&nbsp;...&nbsp;&rbrack;</p>
