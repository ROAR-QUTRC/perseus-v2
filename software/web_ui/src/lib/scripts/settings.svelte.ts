...
export type WidgetSettingsType = Record<
  string,
  Record<string, SettingsFieldType>
>;
...
export class WidgetSettings<T extends WidgetSettingsType = WidgetSettingsType> {
  ...
  getValue<G extends keyof T = keyof T, F extends keyof T[G] = keyof T[G]>(
    ...
    if (!setting) {
      throw new Error(
        `Setting not found: ${group as string}.${field as string}`,
      );
    }
  ...
  >(group: G, field: F): string | null | undefined {
    ...
    if (setting.type !== "button") {
      throw new Error(
        `Setting is not a button: ${group as string}.${field as string}`,
      );
    }
  ...
  >(group: G, field: F, option: { value: string; label: string }): void {
    ...
    if (setting.type !== "select") {
      throw new Error(
        `Setting is not a select: ${group as string}.${field as string}`,
      );
    }
