...
export const resourceMonitor = (socket) => {
	if (!running) {
		timeoutId = setInterval(async () => {
			socket.emit('monitor-data', { cpu: await cpuData(), memory: await memoryData(), network: await networkData() });
		}, 1000);
		running = true;
	}
};
...
const cpuData = () => { 
	return new Promise((resolve, reject) => {
		exec('cat /proc/stat', (error, stdout, stderr) => {
			...
			const active = times[0] + times[1] + times[2] + times[5] + times[6] + times[7]; // calculate active time
...
const memoryData = () => {
	return new Promise((resolve, reject) => {
		exec('free -tb', (error, stdout, stderr) => {
			...
			resolve({ total: { total: total[0], used: total[1] }, normal: { total: normal[0], used: normal[1] }, swap: { total: swap[0], used: swap[1] } });
		});
	});
}
...
const networkData = () => {
	return new Promise(async (resolve, reject) => {
		exec('ls /sys/class/net/', async (_, stdout) => {
			const networkIf = stdout.split('\n').filter((networkIf) => networkIf !== '');

			let networkData = {};
			for (const network of networkIf) {
				const data = await execPromise(`cat /sys/class/net/${network}/statistics/rx_bytes && cat /sys/class/net/${network}/statistics/tx_bytes`);
