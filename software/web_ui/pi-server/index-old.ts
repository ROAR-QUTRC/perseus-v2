...
import config from './config-old.json';
...
watch('/dev/v4l/by-id', (eventType, filename) => {
	console.log(`[Camera server] Detected change in /dev/v4l/by-id: '${eventType}' on '${filename}'`);
});
...
socket.on('connect_error', (error) => {
	console.log('[Camera server][Socket] Connection error, please check the config. Retrying...');
});
...
let cameras: string[] = fs.readdirSync('/dev/v4l/by-id').filter((file) => file.endsWith('-index0')); // index 0 is the video and index 1 is the device metadata
...
socket.on('camera-event', (event: CameraEventType) => {
	...
	switch (event.action) {
		...
		case 'request-stream':
			...
			let index = gstInstances.findIndex((instance) => instance.device === event.data.device);
			...
			if (index !== -1) {
				...
				if (event.data.resolution) {
					...
					console.log(`[Camera server] ${event.data.device} is already streaming`);
					break;
				}
			} else {
				// start the stream
