# Automatically generated by: ros2nix --output-dir=ros_ws/nix-packages --output-as-nix-pkg-name --no-default --distro jazzy
{ lib, buildRosPackage, ament-cmake, ament-lint-auto, ament-lint-common, geometry-msgs, nav2-msgs, perseus-interfaces, rclcpp, rclcpp-action }:
buildRosPackage rec {
  pname = "ros-jazzy-perseus-autonomy-bridge";
  version = "0.0.0";

  src = ./../src/perseus_autonomy_bridge;

  buildType = "ament_cmake";
  buildInputs = [ ament-cmake ];
  checkInputs = [ ament-lint-auto ament-lint-common ];
  propagatedBuildInputs = [ geometry-msgs nav2-msgs perseus-interfaces rclcpp rclcpp-action ];
  nativeBuildInputs = [ ament-cmake ];

  meta = {
    description = "Autonomy Widget Bridge";
    license = with lib.licenses; [ "TODO-License-declaration" ];
  };
}
