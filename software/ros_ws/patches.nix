...
let
  rosOverlay = rosFinal: rosPrev: {
    ...
    rtabmap = prev.rtabmap.overrideAttrs (
      {
        propagatedBuildInputs ? [ ],
        cmakeFlags ? [ ],
        ...
      }:
      {
        ...
        postInstall = ''
          find $out/lib/rtabmap-0.21 -type f -exec sed -i -e "/;VTK::GUISupportQt/d" {} \;
        '';
        propagatedBuildInputs = propagatedBuildInputs ++ [ rosFinal.librealsense2 ];
        patches = [
          ./patches/rtabmap/rtabmap.patch
        ];
      }
    );
    rtabmap-conversions = rosPrev.rtabmap.overrideAttrs (
      {
        propagatedBuildInputs ? [ ],
        ...
      }:
      {
        ...
        src = rosPrev.rtabmap-conversions.src;
        propagatedBuildInputs =
          with final;
          propagatedBuildInputs
          ++ rosPrev.rtabmap-conversions.propagatedBuildInputs
          ++ [
            qt5.qtbase
            octomap
            rosFinal.cv-bridge
        ...
        #       octomap
        #     ];
        patches = [ ./patches/rtabmap/rtabmap-conversions.patch ];
        ROS_DISTRO = rosDistro;
      }
    );
    rtabmap-sync = rosPrev.rtabmap-sync.overrideAttrs (
      {
        ...
      }:
      {
    ...
    fast-lio = rosPrev.fast-lio.overrideAttrs (
      { ... }:
      {
        src = final.fetchFromGitHub {
          ...
          sha256 = "sha256-chnAIRkSQjoXqg9K9s1JVOrNdFtEzFztOFUYnbXkZyI=";
