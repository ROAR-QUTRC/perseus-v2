...
// created internal node to handle subscriptions and publishers
  ...
  if (_latest_rsbl_status.size() >= 14) // At 7 bits per servo
  {
      ...
      for(size_t i=0; i<info_.joints.size(); ++i) {
          if(info_.joints[i].name == "shoulder_tilt") _hw_states_position[i] = tilt_rad;
          if(info_.joints[i].name == "shoulder_pan") _hw_states_position[i] = pan_rad;
      }
  }
  ...
  if (_rsbl_publisher)
  {
      ...
      for(size_t i=0; i<info_.joints.size(); ++i) {
          if(info_.joints[i].name == "shoulder_pan") {
              ...
              if(!std::isnan(_hw_commands_velocity[i])) pan_vel_cmd = _hw_commands_velocity[i];
          }
          if(info_.joints[i].name == "shoulder_tilt") {
              tilt_cmd = _hw_commands[i];
              if(!std::isnan(_hw_commands_velocity[i])) tilt_vel_cmd = _hw_commands_velocity[i];
          }
      }
