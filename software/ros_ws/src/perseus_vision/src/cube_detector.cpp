  ...
  const std_msgs::msg::Header & header)
{
  ...
  for (const auto & d : detections) {
    ...
    // label: "blue 0.97"
    const std::string label = CLASS_NAMES[d.class_id]
      + " " + std::to_string(d.confidence).substr(0, 4);

    // centre coordinates label: "cx:320 cy:240"
    const int cx = d.bbox.x + d.bbox.width  / 2;
    const int cy = d.bbox.y + d.bbox.height / 2;
    const std::string centre_label = "cx:" + std::to_string(cx)
                                   + " cy:" + std::to_string(cy);
    ...
    // draw centre coordinates below class label
    cv::putText(annotated, centre_label,
...
//     // bounding box centre + size in pixel coordinates
