...
namespace
{
    ...
    std::vector<Detection> nms(const std::vector<Detection>& dets, float iou_the)
    {
        ...
        {
            auto& v = kv.second;
            std::sort(v.begin(), v.end(), [](const Detection& a, const Detection& b)
            ...
            for (size_t i = 0; i < v.size(); ++i)
            {
                ...
                for (size_t j = i + 1; j < v.size(); ++j)
                {
                    if (removed[j])
                        continue;
                    if (IoU(v[i].box, v[j].box) > iou_the)
                        removed[j] = true;
                }
            }
        }
...
private:
    void imageCb(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        // for Simulation only, CONF_THE = 0.30f
        // for Real world, CONF_THE = 0.50f
        constexpr float CONF_THE = 0.30f;  // tune as needed
        ...
        for (int i = 0; i < num_preds; ++i)
        {
            ...
            int it1 = std::max(0, std::min(orig_h - 1, static_cast<int>(std::round(y1))));
            int ix2 = std::max(0, std::min(orig_w - 1, static_cast<int>(std::round(x2))));
            int it2 = std::max(0, std::min(orig_h - 1, static_cast<int>(std::round(y2))));

            int bw = std::max(1, ix2 - ix1);
            int bh = std::max(1, it2 - it1);

            dets.push_back({cv::Rect(ix1, it1, bw, bh), best_cls, conf});
        }
