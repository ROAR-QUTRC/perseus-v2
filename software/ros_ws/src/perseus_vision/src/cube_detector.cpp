    ...
    : Node("cube_detector")
{
    ...
    this->declare_parameter("camera_matrix", std::vector<double>{530.4, 0.0, 320.0, 0.0, 530.4, 240.0, 0.0, 0.0, 1.0});
...
double CubeDetector::calculateDistance(const geometry_msgs::msg::PoseStamped& pose1,
...
void CubeDetector::updateOrCreateCube(const geometry_msgs::msg::PoseStamped& world_pose)
{
...
void CubeDetector::imageCb(const sensor_msgs::msg::Image::SharedPtr msg)
{
    ...
    for (const auto& d : final_dets)
    {
        ...
        float Z_cv = Z;                         // Forward from camera
            ...
            cam_pose.header.frame_id = camera_frame_;        // Apply same OpenCV to ROS coordinate transformation as ArUco detector
        ...
        cam_pose.pose.orientation.w = 1.0;  // identity quaternion
            ...
            if (publish_tf_)
            {
                try
                {
                    ...
                    geometry_msgs::msg::PoseStamped odom_pose = tf_buffer_->transform(cam_pose, tf_output_frame_, tf2::durationFromSec(0.1));
