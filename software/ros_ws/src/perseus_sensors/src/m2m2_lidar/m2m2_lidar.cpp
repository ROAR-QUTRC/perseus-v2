...
vector<std::tuple<float, float, bool>> M2M2Lidar::_decode_laser_points(const string& base64Encoded)
{
    vector<std::tuple<float, float, bool>> points;

    vector<uint8_t> decoded = _decode_base64(base64Encoded);
...
void M2M2Lidar::_read_sensor_data()
{
    ...
    auto raw_points = _decode_laser_points(base64Points);
    if (raw_points.empty())
    {
