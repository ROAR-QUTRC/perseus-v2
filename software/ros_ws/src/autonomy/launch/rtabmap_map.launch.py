    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "rviz",
                default_value="false",
                description="Launch RViz point cloud visualization (optional).",
            ),
            DeclareLaunchArgument(
                "use_sim_time",
                default_value="false",
            ...
            DeclareLaunchArgument(
                "cfg",
                default_value="",
                description="Path to RTAB-Map config file (*.ini).",
            ),
            DeclareLaunchArgument(
                "gui_cfg",
            ...
            DeclareLaunchArgument(
                "odom_frame_id",
                default_value="odom",
                description="Odometry frame id. If set, TF is used to get odometry instead of the topic.",
            ),
            DeclareLaunchArgument(
                "map_frame_id",
                default_value="map",
            ...
            DeclareLaunchArgument(
                "map_topic", default_value="map_3d", description="Map topic name."
            ),
            DeclareLaunchArgument(
                "publish_tf_map",
                default_value="true",
                description="Publish TF between map and odometry frames.",
            ),
            # General
            DeclareLaunchArgument(
                "namespace",
            ...
            DeclareLaunchArgument(
                ...
                description="Where the map is saved/loaded.",
            ),
            DeclareLaunchArgument(
                "topic_queue_size",
                default_value="5",
            ...
            DeclareLaunchArgument(
                "qos",
                default_value="0",
                description="General QoS: 0=system default, 1=Reliable, 2=Best Effort.",
            ),
            DeclareLaunchArgument(
                "wait_for_transform",
                default_value="0.2",
                description="Wait for transform timeout (seconds).",
            ),
            DeclareLaunchArgument(
                "rtabmap_args",
                default_value="",
                description="Extra RTAB-Map arguments (e.g., --delete_db_on_start).",
            ),
            DeclareLaunchArgument(
                "launch_prefix",
                default_value="",
                description='Node launch prefix, e.g., "xterm -e gdb -ex run --args".',
            ),
            DeclareLaunchArgument(
                "output",
            ...
            DeclareLaunchArgument(
                "use_action_for_goal",
                default_value="false",
                description="Connect to nav2's navigate_to_pose action server.",
            ),
            ...
            DeclareLaunchArgument(
                "ground_truth_base_frame_id",
                default_value="",
                description='e.g., "tracker"',
            ),
            # Synchronization
            DeclareLaunchArgument(
                "approx_sync",
                default_value="true",
                description="Use approximate time synchronization policy.",
            ),
            DeclareLaunchArgument(
                "approx_sync_max_interval",
                default_value="0.08",
                description="Max interval (sec) for approx_sync. 0 = infinite.",
            ),
            # RGB-D topics
            DeclareLaunchArgument(
                "rgb_topic",
            ...
            DeclareLaunchArgument(
                ...
                description="Stereo camera namespace.",
            ),
            DeclareLaunchArgument(
                "left_image_topic",
                default_value=[
            ...
            DeclareLaunchArgument(
                "rgbd_sync",
                default_value="true",
                description="Pre-sync rgb/depth/camera_info topics.",
            ),
            DeclareLaunchArgument(
                "approx_rgbd_sync",
                default_value="true",
                description="Use approximate sync for RGBD. false=exact.",
            ),
            DeclareLaunchArgument(
                "subscribe_rgbd",
                default_value=LaunchConfiguration("rgbd_sync"),
                description="Subscribe to pre-synced RGBDImage topic.",
            ),
            DeclareLaunchArgument(
                "rgbd_topic",
                default_value="rgbd_image",
                description="Pre-synced RGBD topic name.",
            ),
            # 1 is mm, 1000 is m, etc.
            DeclareLaunchArgument(
                "depth_scale",
                default_value="1.0",
                description="Depth image scale factor.",
            ),
            # Image compression
            DeclareLaunchArgument(
                "compressed",
                default_value="false",
                description="Subscribe to compressed image topics.",
            ),
            DeclareLaunchArgument(
                "rgb_image_transport",
                default_value="compressed",
                description="RGB image transport type.",
            ),
            DeclareLaunchArgument(
                "depth_image_transport",
                default_value="compressedDepth",
                description="Depth image transport type.",
            ),
            # LiDAR
            DeclareLaunchArgument(
                "subscribe_scan",
                default_value="false",
                description="Subscribe to laser scan topic.",
            ),
            DeclareLaunchArgument(
                "scan_topic",
                default_value="/scan",
            ...
            DeclareLaunchArgument(
                ...
                default_value="false",
                description="Subscribe to point cloud scan topic.",
            ),
            DeclareLaunchArgument(
                "scan_cloud_topic",
                default_value="/scan_cloud",
                description="Point cloud scan topic name.",
            ),
            DeclareLaunchArgument(
                "scan_normal_k",
                default_value="0",
                description="Number of neighbors for scan normal estimation.",
            ),
            # Odometry (consumed by SLAM node, not launched here)
            DeclareLaunchArgument(
                "visual_odometry",
                default_value="false",
                description="Whether an external visual odometry node is running (for subscribe_odom_info).",
            ),
            DeclareLaunchArgument(
                "icp_odometry",
                default_value="false",
                description="Whether an external ICP odometry node is running (for subscribe_odom_info).",
            ),
            DeclareLaunchArgument(
                "odom_topic",
            ...
            DeclareLaunchArgument(
                "odom_tf_angular_variance",
                default_value="0.01",
                description="Default angular variance when TF is used for odometry.",
            ),
            DeclareLaunchArgument(
                "odom_tf_linear_variance",
                default_value="0.001",
                description="Default linear variance when TF is used for odometry.",
            ),
            DeclareLaunchArgument(
                "odom_sensor_sync",
                default_value="false",
                description="Enable odometry sensor sync.",
            ),
            # IMU
            DeclareLaunchArgument(
                "imu_topic",
                default_value="/imu/data",
                description="IMU topic for gravity constraints.",
            ),
            # User data
            DeclareLaunchArgument(
                "subscribe_user_data",
                default_value="false",
                description="Subscribe to user data topic (synchronized).",
            ),
            DeclareLaunchArgument(
                "user_data_topic",
                default_value="/user_data",
                description="User data topic name.",
            ),
            DeclareLaunchArgument(
                "user_data_async_topic",
                default_value="/user_data_async",
                description="User data async topic name.",
            ),
            # GPS
            DeclareLaunchArgument(
                "gps_topic",
                default_value="/gps/fix",
                description="GPS topic for graph optimization.",
            ),
            # Tags / Landmarks
            DeclareLaunchArgument(
                "tag_topic",
            ...
            DeclareLaunchArgument(
                ...
                description="Linear variance for tag landmarks.",
            ),
            DeclareLaunchArgument(
                "tag_angular_variance",
                default_value="9999.0",
                description="Angular variance for tag landmarks (>=9999 ignores rotation).",
            ),
            DeclareLaunchArgument(
                "fiducial_topic",
                default_value="/fiducial_transforms",
                description="aruco_detect fiducial transforms topic.",
            ),
            # Environment sensor
            DeclareLaunchArgument(
                "env_sensor_topic",
                default_value="/env_sensor",
                description="rtabmap_msgs/EnvSensor topic.",
            ),
