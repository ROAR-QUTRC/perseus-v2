...
double HeatmapCell::rssi_avg() const {
  if (rssi_count == 0) return -100.0;
  return static_cast<double>(rssi_sum) / rssi_count;
}

// -- HeatmapGrid --

HeatmapGrid::HeatmapGrid(const Config& cfg) : cfg_(cfg) {}
...
HeatmapGrid::find_nearest_good_signal(double current_x, double current_y,
  ...
  if (!best_pos) return std::nullopt;
...
std::tuple<float, float, float> HeatmapGrid::rssi_to_colour(double rssi_avg) {
  if (rssi_avg >= -50) return {0.0f, 1.0f, 0.0f};       // Green
  if (rssi_avg >= -60) return {0.5f, 1.0f, 0.0f};       // Yellow-green
  if (rssi_avg >= -70) return {1.0f, 1.0f, 0.0f};       // Yellow
  if (rssi_avg >= -80) return {1.0f, 0.5f, 0.0f};       // Orange
  if (rssi_avg >= -90) return {1.0f, 0.0f, 0.0f};       // Red
...
void HeatmapGrid::save_to_disk(const std::string& save_directory,
  ...
  if (!save_dir.empty() && save_dir[0] == '~') {
    ...
    if (home) save_dir = std::string(home) + save_dir.substr(1);
  }
