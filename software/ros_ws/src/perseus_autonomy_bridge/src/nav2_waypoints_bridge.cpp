...
#include "perseus_autonomy_bridge/srv/run_waypoints.hpp"
#include "perseus_autonomy_bridge/srv/cancel_waypoints.hpp"
...
static std::vector<Waypoint> load_waypoints_yaml(const std::string & path)
{
  ...
  while (std::getline(in, line)) {
    ...
    if (line == "waypoints:") continue;

    if (line.rfind("- ", 0) == 0) {
      if (cur.has_value()) out.push_back(*cur);
      cur = Waypoint{};
      ...
      if (!rest.empty()) {
        ...
        if (parse_kv(rest, k, v)) {
          ...
          else if (k == "x") cur->x = std::stod(v);
          else if (k == "y") cur->y = std::stod(v);
          else if (k == "yaw") cur->yaw = std::stod(v);
        }
      }
      continue;
    }
  ...
  for (size_t i = 0; i < out.size(); i++) {
    if (out[i].name.empty()) out[i].name = "WP" + std::to_string(i + 1);
  }
  if (out.empty()) {
...
public:
  ...
  : Node("nav2_waypoints_bridge")
  {
    ...
    srv_run_ = create_service<perseus_autonomy_bridge::srv::RunWaypoints>(
      "/autonomy/run_waypoints",
      std::bind(&Nav2WaypointsBridge::on_run, this, std::placeholders::_1, std::placeholders::_2));

    srv_cancel_ = create_service<perseus_autonomy_bridge::srv::CancelWaypoints>(
...
private:
  ...
  rclcpp::Service<perseus_autonomy_bridge::srv::RunWaypoints>::SharedPtr srv_run_;
  rclcpp::Service<perseus_autonomy_bridge::srv::CancelWaypoints>::SharedPtr srv_cancel_;

  void on_run(
    const std::shared_ptr<perseus_autonomy_bridge::srv::RunWaypoints::Request> req,
    std::shared_ptr<perseus_autonomy_bridge::srv::RunWaypoints::Response> resp)
  {
    try {
  ...
  void on_cancel(
    const std::shared_ptr<perseus_autonomy_bridge::srv::CancelWaypoints::Request>,
    std::shared_ptr<perseus_autonomy_bridge::srv::CancelWaypoints::Response> resp)
  {
    if (!active_goal_) {
      resp->success = true;
