...
private:
    // Robot constants
    static constexpr int    COUNTS_PER_REV  = 6400;  // 64 CPR * 4 edges * 100:1
    static constexpr double RADS_PER_COUNT  =
        (2.0 * std::numbers::pi) / static_cast<double>(COUNTS_PER_REV);
    // Max output shaft speed ~100 RPM no-load = 10.47 rad/s
    ...
    // Drive wheel joints
    // index 0 = left_wheel_joint  (Motor A / Encoder A)
    // index 1 = right_wheel_joint (Motor B / Encoder B)
    ...
    // GPIO — space motor
    double space_motor_cmd_ = 0.0;   // -100.0 .. 100.0
