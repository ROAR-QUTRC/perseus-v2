...
let
  ...
  # pass the top-level (nixpkgs) sophus so vikit avoids the heavy ros-jazzy-sophus, and
  # pkg-config/mpi since neither is in the ROS package set (mesh_msgs_conversions'
  # CMakeLists.txt find_package()s both directly, unused, copy-pasted from lvr2's own)
  # this `final` is the nixpkgs one, which is the whole point of passing it from here
  thirdPartyPackages = import ./third-party-packages/overlay.nix {
    inherit (final) sophus pkg-config mpi;
  };
