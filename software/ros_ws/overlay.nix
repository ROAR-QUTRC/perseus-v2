...
let
  ...
  # pass the top-level (nixpkgs) sophus so vikit avoids the heavy ros-jazzy-sophus
  # this `final` is the nixpkgs one, which is the whole point of passing it from here
  thirdPartyPackages = import ./third-party-packages/overlay.nix { inherit (final) sophus; };
