...
void ST3215ServoReaderMini::writePosition(uint8_t servo_id, uint16_t position)
{
    ...
    // Optionally try to read status packet, but don't require it
    try
    {
        // Status packet is typically 6 bytes
        std::vector<uint8_t> response(6);

        // Use read_some with a small timeout to check if there's data available
        boost::system::error_code ec;
        size_t bytes_read = _serial_port.read_some(boost::asio::buffer(response), ec);

        // If we got data and there was no error, process it
        if (bytes_read > 0 && !ec)
        {
            // Process status packet if needed
        }
    }
    catch (...)
    {
        // Ignore any errors when trying to read the status packet
        // Some servos might not respond with a status packet
    }
}
