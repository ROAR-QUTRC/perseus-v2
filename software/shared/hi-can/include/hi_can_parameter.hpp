...
namespace hi_can::parameters
{
    ...
    public:
        virtual std::vector<uint8_t> serializeData()
        {
            ...
            dataBuf.resize(sizeof(*this));
            std::copy_n(reinterpret_cast<uint8_t*>(this), sizeof(*this), dataBuf.begin());
            return dataBuf;
        }
    };
    ...
    {
    public:
        virtual void deserializeData(const std::vector<uint8_t>& serializedData)
        {
            if (serializedData.size() != sizeof(*this))
                throw std::invalid_argument("Data size does not match");
            std::copy(serializedData.begin(), serializedData.end(), reinterpret_cast<uint8_t*>(this));
        }
    };
