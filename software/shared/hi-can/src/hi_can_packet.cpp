...
void PacketManager::setTransmissionConfig(const addressing::flagged_address_t& address, const transmission_config_t& config)
{
...
void PacketManager::setTransmissionGenerator(const addressing::flagged_address_t& address, const data_generator_t& generator)
{
    if (const auto it = _transmissions.find(address); it != _transmissions.end())
        it->second.config.generator = generator;
}

void PacketManager::setTransmissionInterval(const addressing::flagged_address_t& address, const std::chrono::steady_clock::duration& interval)
{
    if (const auto it = _transmissions.find(address); it != _transmissions.end())
        it->second.config.interval = interval;
}

std::optional<PacketManager::transmission_config_t> PacketManager::getTransmissionConfig(const addressing::flagged_address_t& address)
{
    if (const auto it = _transmissions.find(address); it != _transmissions.end())
        return it->second.config;
    return std::nullopt;
...
void PacketManager::removeTransmission(const addressing::flagged_address_t& address)
{
