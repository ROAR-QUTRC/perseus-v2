...
void ST3215ServoReader::writePosition(uint8_t servo_id, uint16_t position) 
{
    ...
    if (read >= 4 && response[0] == 0xFF && response[1] == 0xFF && 
        ...
        if (response[4] != 0x00) {
            std::string error = "Servo errors:";
            if (response[4] & 0x01) error += " Input Voltage";
            if (response[4] & 0x02) error += " Angle Limit";
            if (response[4] & 0x04) error += " Overheating";
            if (response[4] & 0x08) error += " Range";
            if (response[4] & 0x10) error += " Checksum";
            if (response[4] & 0x20) error += " Overload";
            if (response[4] & 0x40) error += " Instruction";
            throw std::runtime_error(error);
        }
    }
}
