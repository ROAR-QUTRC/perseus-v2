...
private:
     ...
     * @param id Servo ID
     * @param address Memory // Add this to perseus-arm-teleop.hpp
#pragma once
...
ddress to write to* @param data Data bytes to write* @ return Vector containing the complete command packet * /
    std::vector<uint8_t> _createWriteCommand(uint8_t id, uint8_t address, const std::vector<uint8_t>& data);
