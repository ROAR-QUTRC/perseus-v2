...
{ stdenv, lib, colcon, pythonPackages, rosDistro, rosVersion }:
{ doCheck ? false # TODO: implement colcon test phase
  ...
  # correctly.
, dontWrapQtApps ? true, nativeBuildInputs ? [ ], CXXFLAGS ? "", postFixup ? ""
, passthru ? { }, separateDebugInfo ? true, ... }@args:
...
stdenv.mkDerivation (args // {
  ...
  nativeBuildInputs = nativeBuildInputs
    ++ [ colcon ]; # python3Packages.colcon-core ];
