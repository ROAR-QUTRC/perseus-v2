# This derivation was mostly copied from https://github.com/NixOS/nixpkgs/blob/nixos-24.11/pkgs/development/libraries/freeimage/default.nix because it is abandoned. It also needed some fixes to make sure it linked to the correct version of libjpeg
{
  lib,
  stdenv,
  fetchsvn,
...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    substituteInPlace Makefile.gnu \
      --replace "pkg-config" "$PKG_CONFIG"
    substituteInPlace Makefile.gnu \
      --replace "LIBJPEG_LIBRARIES" "-L${libjpeg.out}/lib -l:libjpeg.a"
    substituteInPlace Makefile.gnu \
      --replace "LIBJPEG_INCLUDES" "-I${libjpeg.dev}/include"
  ''
